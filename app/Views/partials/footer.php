<footer class="bg-primary-900 text-white">

    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10">

        <div class="lg:col-span-1">
            <a href="<?= url('/') ?>" class="inline-block mb-4">
                <img src="<?= asset('images/medizinar-care-logo-footer.webp') ?>" alt="Medizinar Care – Home Healthcare Kerala" class="h-10 w-auto rounded-md bg-white p-1">
            </a>
            <p class="text-primary-200 text-sm leading-relaxed mb-5">
                Compassionate and reliable home healthcare services for patients, elderly individuals, and families — delivered with dignity and care.
            </p>
            <div class="flex items-center gap-3">
                <a href="<?= h(whatsapp_link(WHATSAPP_NUM)) ?>" target="_blank" rel="noopener" aria-label="WhatsApp"
                    class="w-9 h-9 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    </svg>
                </a>
                <a href="https://www.facebook.com/share/1bADZmQVno/" target="_blank" rel="noopener" aria-label="Facebook"
                    class="w-9 h-9 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                </a>
                <a href="https://www.instagram.com/medizinarcare?utm_source=qr&igsh=MTZhdXhvdmJiZjNjdQ==" target="_blank" rel="noopener" aria-label="Instagram"
                    class="w-9 h-9 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                    </svg>
                </a>
                <a href="https://www.youtube.com/@MedizinarCare" target="_blank" rel="noopener" aria-label="YouTube"
                    class="w-9 h-9 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                    </svg>
                </a>
                <a href="https://www.linkedin.com/company/medizinar/" target="_blank" rel="noopener" aria-label="LinkedIn"
                    class="w-9 h-9 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                    </svg>
                </a>
                <a href="#" target="_blank" rel="noopener" aria-label="Google Business"
                    class="w-9 h-9 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12.545 10.239v3.821h5.445c-.712 2.315-2.647 3.972-5.445 3.972a6.033 6.033 0 110-12.064 5.963 5.963 0 014.123 1.632l2.884-2.884A9.969 9.969 0 0012.545 2C7.021 2 2.543 6.477 2.543 12s4.478 10 10.002 10c8.396 0 10.249-7.85 9.426-11.748l-9.426-.013z" />
                    </svg>
                </a>
            </div>
        </div>

        <div>
            <h3 class="font-semibold text-white mb-5 text-base">Quick Links</h3>
            <div class="w-8 h-0.5 mb-4 rounded" style="background:#a5781e"></div>
            <ul class="space-y-2.5">
                <li><a href="<?= url('/') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Home</a></li>
                <li><a href="<?= url('/about') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">About Us</a></li>
                <li><a href="<?= url('/services') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Our Services</a></li>
                <li><a href="<?= url('/team') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Our Team</a></li>
                <li><a href="<?= url('/contact') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Contact Us</a></li>
                <li><a href="<?= url('/appointment') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Make an Appointment</a></li>
                <li><a href="<?= url('/blog') ?>" target="_blank" rel="noopener" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Blog</a></li>
                <li><a href="<?= url('/faq') ?>" target="_blank" rel="noopener" class="text-primary-200 hover:text-accent-light text-sm transition-colors">FAQ</a></li>
            </ul>
        </div>

        <div>
            <h3 class="font-semibold text-white mb-5 text-base">Legal</h3>
            <div class="w-8 h-0.5 mb-4 rounded" style="background:#a5781e"></div>
            <ul class="space-y-2.5">
                <li><a href="<?= url('/privacy-policy') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Privacy Policy</a></li>
                <li><a href="<?= url('/terms-and-conditions') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Terms &amp; Conditions</a></li>
                <li><a href="<?= url('/disclaimer') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Disclaimer</a></li>
                <li><a href="<?= url('/refund-policy') ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Refund Policy</a></li>
            </ul>
        </div>

        <div>
            <h3 class="font-semibold text-white mb-5 text-base">Our Services</h3>
            <div class="w-8 h-0.5 mb-4 rounded" style="background:#a5781e"></div>
            <ul class="space-y-2.5">
                <li><a href="<?= url('/services') ?>#bedside" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Bedside Patient Care</a></li>
                <li><a href="<?= url('/services') ?>#elderly" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Elderly Care</a></li>
                <li><a href="<?= url('/services') ?>#mother-baby" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Mother &amp; Baby Care</a></li>
                <li><a href="<?= url('/services') ?>#housemaid" class="text-primary-200 hover:text-accent-light text-sm transition-colors">House Maid Services</a></li>
                <li><a href="<?= url('/services') ?>#quick-support" class="text-primary-200 hover:text-accent-light text-sm transition-colors">Quick Support Services</a></li>
                <li><a href="<?= url('/services') ?>#nri" class="text-primary-200 hover:text-accent-light text-sm transition-colors">NRI Parent Care</a></li>
            </ul>
        </div>

        <div>
            <h3 class="font-semibold text-white mb-5 text-base">Contact Info</h3>
            <div class="w-8 h-0.5 mb-4 rounded" style="background:#a5781e"></div>
            <ul class="space-y-4">
                <li class="flex gap-3 items-start">
                    <svg class="w-4 h-4 text-accent-light mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                    </svg>
                    <span class="text-primary-200 text-sm leading-relaxed">
                        <?= ADDRESS_LINE1 ?><br>
                        <?= ADDRESS_LINE2 ?><br>
                        <?= ADDRESS_LINE3 ?>
                    </span>
                </li>
                <li class="flex gap-3 items-center">
                    <svg class="w-4 h-4 text-accent-light shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                    </svg>
                    <a href="tel:<?= PHONE ?>" class="text-primary-200 hover:text-accent-light text-sm transition-colors"><?= PHONE_DISPLAY ?></a>
                </li>
                <li class="flex gap-3 items-center">
                    <svg class="w-4 h-4 text-accent-light shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                        <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                    </svg>
                    <a href="mailto:<?= EMAIL ?>" class="text-primary-200 hover:text-accent-light text-sm break-all transition-colors"><?= EMAIL ?></a>
                </li>
                <li>
                    <a href="<?= url('/appointment') ?>"
                        class="inline-flex items-center gap-2 bg-accent hover:bg-accent-hover text-white font-semibold text-xs px-4 py-2.5 rounded-lg transition-colors mt-1">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        Make an Appointment
                    </a>
                </li>
            </ul>
        </div>

    </div>

    <div class="border-t" style="border-color:#a5781e">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p class="text-primary-300 text-xs">
                &copy; <?= date('Y') ?> <?= SITE_NAME ?>. All rights reserved.
            </p>
            <div class="flex items-center gap-2.5">
                <a href="<?= h(whatsapp_link(WHATSAPP_NUM)) ?>" target="_blank" rel="noopener" aria-label="WhatsApp"
                    class="w-7 h-7 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    </svg>
                </a>
                <a href="https://www.facebook.com/share/1bADZmQVno/" target="_blank" rel="noopener" aria-label="Facebook"
                    class="w-7 h-7 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                </a>
                <a href="https://www.instagram.com/medizinarcare?utm_source=qr&igsh=MTZhdXhvdmJiZjNjdQ==" target="_blank" rel="noopener" aria-label="Instagram"
                    class="w-7 h-7 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                    </svg>
                </a>
                <a href="https://www.youtube.com/@MedizinarCare" target="_blank" rel="noopener" aria-label="YouTube"
                    class="w-7 h-7 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                    </svg>
                </a>
                <a href="https://www.linkedin.com/company/medizinar/" target="_blank" rel="noopener" aria-label="LinkedIn"
                    class="w-7 h-7 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                    </svg>
                </a>
                <a href="#" target="_blank" rel="noopener" aria-label="Google Business"
                    class="w-7 h-7 rounded-full bg-primary-700 hover:bg-accent flex items-center justify-center transition-colors">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12.545 10.239v3.821h5.445c-.712 2.315-2.647 3.972-5.445 3.972a6.033 6.033 0 110-12.064 5.963 5.963 0 014.123 1.632l2.884-2.884A9.969 9.969 0 0012.545 2C7.021 2 2.543 6.477 2.543 12s4.478 10 10.002 10c8.396 0 10.249-7.85 9.426-11.748l-9.426-.013z" />
                    </svg>
                </a>
            </div>
        </div>
    </div>

</footer>