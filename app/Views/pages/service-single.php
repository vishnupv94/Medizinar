 ...
 * Variables: $service (array), $slug (string)
 */
...
$accentShadow = $isGreen ? 'rgba(23,107,35,0.15)' : 'rgba(171,126,34,0.15)';
?>

<?php partial('inner-hero', [
    'heroTitle'       => h($service['h1']),
    'heroDescription' => h($service['heroDesc']),
    'breadcrumb'      => h($service['h1']),
]) ?>


<!-- AI-Readable Factual Intro (helps AI search & featured snippets) -->
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div class="fade-in-up">
                ...
                <h2 class="text-2xl font-bold text-gray-800 mb-4"><?= h($service['intro']['what']) ?></h2>
                <p class="text-gray-600 leading-relaxed mb-6"><?= h($service['intro']['body']) ?></p>
        ...
        <div class="rounded-2xl overflow-hidden fade-in-up" style="background:#0c2912;position:relative;min-height:160px">
            ...
            <div class="relative z-10 p-8 sm:p-10 flex flex-col sm:flex-row items-center justify-between gap-6">
                ...
                <div class="flex flex-col sm:flex-row gap-3 shrink-0">
                    <a href="<?= url('/appointment') ?>?service=<?= h($service['serviceParam']) ?>" class="btn-primary whitespace-nowrap">
                        Book This Service
                    </a>
...
<!-- FAQ Section (for featured snippets & PAA) -->
            ...
            <?php foreach ($service['faqs'] as $i => $faq): ?>
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    <button class="w-full text-left px-6 py-4 flex items-center justify-between gap-4 font-semibold text-gray-800 hover:bg-gray-50 transition-colors"
                        ...
                        <span><?= h($faq['q']) ?></span>
                        <svg class="w-4 h-4 shrink-0 transition-transform faq-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    <div class="faq-body px-6 pb-4 text-gray-600 text-sm leading-relaxed hidden">
                        <?= h($faq['a']) ?>
                    </div>
                </div>
            <?php endforeach; ?>
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <?php
        $otherServices = [
            'bedside-patient-care' => ['name' => 'Bedside Patient Care', 'badge' => 'Healthcare'],
            'elderly-care'         => ['name' => 'Elderly Care',         'badge' => 'Senior Care'],
            'mother-baby-care'     => ['name' => 'Mother & Baby Care',   'badge' => 'Postnatal'],
            'home-maid-services'   => ['name' => 'Home Maid Services',   'badge' => 'Domestic'],
            'nri-parent-care'      => ['name' => 'NRI Parent Care',      'badge' => 'NRI'],
            'quick-support'        => ['name' => 'Quick Support',        'badge' => 'Short-Term'],
        ];
        unset($otherServices[$slug]);
        ?>
        <div class="grid sm:grid-cols-2 lg:grid-cols-5 gap-4 fade-in-up">
            <?php foreach ($otherServices as $otherSlug => $other): ?>
                <a href="<?= url('/services/' . $otherSlug) ?>"
                    ...
                    <p class="text-gray-800 font-semibold text-sm mt-1 group-hover:text-primary-700 transition-colors"><?= h($other['name']) ?></p>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
