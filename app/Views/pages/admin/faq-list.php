<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <p class="text-sm text-gray-500"><?= $total ?> total FAQ<?= $total === 1 ? '' : 's' ?><?= $q !== '' ? ' matching <strong class="text-gray-700">' . h($q) . '</strong>' : '' ?></p>
        </div>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <form method="GET" action="<?= url('/admin/faqs') ?>" class="flex items-center gap-2">
                <div class="relative">
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z"/>
                    </svg>
                    <input type="text" name="q" value="<?= h($q) ?>" placeholder="Search FAQs…"
                ...
                <?php if ($q !== ''): ?>
                    <a href="<?= url('/admin/faqs') ?>" class="px-3 py-2 text-sm text-gray-500 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Clear</a>
                <?php endif; ?>
            </form>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <?php if (empty($faqs)): ?>
            <p class="px-6 py-12 text-sm text-gray-400 text-center">No FAQs found.</p>
        <?php else: ?>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            ...
                            <th class="px-5 py-3 w-32 text-center">Sort Order</th>
                            <th class="px-5 py-3 w-32">Status</th>
                            <th class="px-5 py-3 w-40">Last Updated</th>
                        ...
                        <?php foreach ($faqs as $faq): ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-5 py-3 text-gray-500 whitespace-nowrap">
                                    <?= date('M j, Y H:i', strtotime($faq->updated_at)) ?>
                                </td>
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="<?= url('/admin/faqs/' . $faq->id . '/edit') ?>" title="Edit"
                                        ...
                                        <form action="<?= url('/admin/faqs/' . $faq->id . '/delete') ?>" method="POST"
                                            data-confirm="Are you sure you want to delete this FAQ?" class="inline">
                                            <?= csrf_field() ?>
    ...
    <?php if ($totalPages > 1): ?>
        ...
        <div class="flex items-center justify-center gap-2">
            <?php if ($page > 1): ?>
                <a href="<?= url('/admin/faqs?page=' . ($page - 1) . $qs) ?>"
