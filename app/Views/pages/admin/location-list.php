<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <p class="text-sm text-gray-500"><?= $total ?> location<?= $total === 1 ? '' : 's' ?><?= $q !== '' ? ' matching <strong class="text-gray-700">' . h($q) . '</strong>' : '' ?></p>
        </div>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <form method="GET" action="<?= url('/admin/locations') ?>" class="flex items-center gap-2">
                <div class="relative">
                    ...
                    <input type="text" name="q" value="<?= h($q) ?>" placeholder="Search locations…"
                ...
                <?php if ($q !== ''): ?>
                    <a href="<?= url('/admin/locations') ?>" class="px-3 py-2 text-sm text-gray-500 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Clear</a>
                <?php endif; ?>
            </form>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <?php if (empty($locations)): ?>
            <p class="px-6 py-12 text-sm text-gray-400 text-center">No locations found.</p>
        <?php else: ?>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php foreach ($locations as $loc): ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-5 py-3">
                                    <a href="<?= url('/admin/locations/' . $loc->id . '/edit') ?>"
                                        class="font-medium text-gray-800 hover:text-primary transition-colors block">
                                        <?= h($loc->name) ?>
                                    </a>
                                    <p class="text-xs text-gray-400 mt-0.5">/location/<?= h($loc->slug) ?></p>
                                </td>
                                <td class="px-5 py-3 text-gray-600"><?= number_format((float)$loc->sitemap_priority, 1) ?></td>
                                <td class="px-5 py-3 text-center text-gray-700"><?= (int) $loc->sort_order ?></td>
                                <td class="px-5 py-3">
                                    <?php if ($loc->status === 'published'): ?>
                                ...
                                <td class="px-5 py-3 text-gray-500 whitespace-nowrap">
                                    <?= date('M j, Y', strtotime($loc->updated_at)) ?>
                                </td>
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="<?= url('/location/' . $loc->slug) ?>" target="_blank" title="View live page"
                                        ...
                                        <a href="<?= url('/admin/locations/' . $loc->id . '/edit') ?>" title="Edit"
                                            class="p-1.5 rounded-md text-gray-400 hover:text-primary hover:bg-primary-50 transition-colors">
                                            <svg class="w-4 h-4" width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <form action="<?= url('/admin/locations/' . $loc->id . '/delete') ?>" method="POST"
                                            data-confirm="Delete '<?= h($loc->name) ?>'? This cannot be undone." class="inline">
                                            <?= csrf_field() ?>
    ...
    <?php if ($totalPages > 1): ?>
        ...
        <div class="flex items-center justify-center gap-2">
            <?php if ($page > 1): ?>
                <a href="<?= url('/admin/locations?page=' . ($page - 1) . $qs) ?>"
